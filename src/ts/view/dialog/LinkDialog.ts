  ...
  getDialogHtml({isExternal, linkData, pageNames}) {
    return `
      <section class="link-editor">
        <div class="labels">
          <label for="link-editor-external" title="External Link" class="link-editor-tab-label first-button fa fa-lg fa-link${
        isExternal ? ' checked ' : ''}"></label>
          <label for="link-editor-internal" title="Link to a page" class="link-editor-tab-label last-button fa fa-lg fa-file"${
        isExternal ? '' : ' checked '}></label>
        </div>
        <input autocomplete="nope" id="link-editor-external" class="link-editor-radio" type="radio" name="link-editor-tab-group"${
        isExternal ? ' checked ' : ''}/>
        <div class="link-editor-tab link-editor-tab-external">
          <div class="link-editor-tab-container">
            ...
            <input autocomplete="nope" id="link-editor-href" class="alertify-text href tabbed" type="url" value="${
        isExternal ? linkData['href'] : ''}">
            <select autocomplete="nope" id="link-editor-target" class="alertify-text target">
              <option${
        linkData['target'] === '' ? ' selected ' : ''} value=""></option>
              <option${
        linkData['target'] === '_self' ? ' selected ' :
                                         ''} value="_self">_self</option>
              <option${
        linkData['target'] === '_blank' ? ' selected ' :
                                          ''} value="_blank">_blank</option>
              <option${
        linkData['target'] === '_parent' ? ' selected ' :
                                           ''} value="_parent">_parent</option>
              <option${
        linkData['target'] === '_top' ? ' selected ' :
                                        ''} value="_top">_top</option>
            </select>
          </div>
        </div>
        <input autocomplete="nope" id="link-editor-internal" class="link-editor-radio" type="radio" name="link-editor-tab-group"${
        !isExternal ? ' checked ' : ''}/>
        ...
        pageNames.map(
            (page) => `<option value="#!${page}"${
                !isExternal && '#!' + page === linkData['href'] ?
                    ' selected ' :
                    ''} >${this.model.page.getDisplayName(page)}</option>`)}
            </select>
          </div>
        </div>
        <div class="link-editor-2col-container">
          <div class="link-editor-2col">
            ...
            <input autocomplete="nope" id="link-editor-title" class="alertify-text title" type="text" value="${
        linkData['title']}">
          </div>
        </div>
        <hr>
        <div class="link-editor-advanced-container">
          ...
          <div class="link-editor-advanced">
            ...
            <input autocomplete="nope" id="link-editor-rel" class="alertify-text rel" type="text" value="${
        linkData['rel']}">
            <label for="link-editor-type">The "type" attribute. Specifies the MIME type of the linked resource.</label>
            <input autocomplete="nope" id="link-editor-type" class="alertify-text type" type="text" value="${
        linkData['type']}">
            ...
            <input autocomplete="nope" id="link-editor-download" class="alertify-text download" type="text" value="${
        linkData['download']}">
          </div>
        </div>
      </section>
