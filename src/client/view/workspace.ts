    ...
    if (isMobileEditor) {
      ...
      if (!this.model.head.getEnableMobile()) {
        SilexNotification.alert(
            'Warning: you are entering the mobile editor, but your website is not configured to support it, so you need to open the menu "File", then "Settings" and "Enable mobile version".',
            () => {});
      }
    } else {
